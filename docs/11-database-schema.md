...
This document defines the initial Supabase/Postgres schema for JS Full IM Studio.
...
Recommended Postgres schema namespace:

```text
public
```

or if using a shared Supabase project with MVP:
...
```text
use public tables with full_im_ prefixes or dedicated schema, depending on existing MVP setup
```

If the MVP is already in the same Supabase project, prefer:

```text
full_im.*
```

to avoid table name collision.
...
## 4. Table: handoff_source_snapshots
...
- experts can read only assigned sections/evidence
- reviewers can read assigned/all review projects
- admins can manage all
- system/service role can run workers
```

Evidence access must be stricter than section access.
...
```text
- all domain entities have tables.
- project membership supports roles.
- section versioning is supported.
- expert assignment and patch are supported.
- gate review is supported.
- exports, Q&A, Golden candidates are supported.
- activity events and AI runs are logged.
